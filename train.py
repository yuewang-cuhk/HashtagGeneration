...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        train_iter = make_dataset_iter(lazily_load_dataset("train"),
                                       fields, opt)
        train_stats = trainer.train(train_iter, epoch, opt.max_src_len, opt.max_conv_len, report_func)
        ...
        valid_iter = make_dataset_iter(lazily_load_dataset("valid"),
                                       fields, opt,
                                       is_train=False)
        valid_stats = trainer.validate(valid_iter, opt.max_src_len, opt.max_conv_len)
        ...
        if trainer.optim.lr <= 1e-6:
            logger.info("Learning rate %g is below 1e-6 at epoch %d. Stop training!!" % (trainer.optim.lr, epoch))
            break
