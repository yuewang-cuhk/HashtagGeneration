...
class RNNTwoEncoder(EncoderBase):
    """ A generic recurrent neural network encoder.
    ...
    def __init__(self, rnn_type, bidirectional, num_layers,
                 hidden_size, dropout=0.0, embeddings=None,
                 use_bridge=False):
        super(RNNTwoEncoder, self).__init__()
        assert embeddings is not None

        num_directions = 2 if bidirectional else 1
        self.combine_two_enc_state = nn.Linear(num_layers * num_directions * hidden_size, hidden_size, bias=True)
    ...
    def forward(self, src, conversation, lengths=None, encoder_state=None):
        "See :obj:`EncoderBase.forward()`"
        src_lengths, conversation_lengths = lengths
        self._check_args(src, src_lengths, encoder_state)
        self._check_args(conversation, conversation_lengths, encoder_state)

        src_emb = self.embeddings(src)
        conv_emb = self.embeddings(conversation)

        # pack src and forward into rnn
        packed_src_emb = src_emb
        if src_lengths is not None and not self.no_pack_padded_seq:
            # Lengths data is wrapped inside a Tensor.
            src_lengths = src_lengths.view(-1).tolist()
            packed_src_emb = pack(src_emb, src_lengths)

        src_memory_bank, src_encoder_final = self.rnn(packed_src_emb, encoder_state)

        if src_lengths is not None and not self.no_pack_padded_seq:
            src_memory_bank = unpack(src_memory_bank)[0]

        # sorted_conv_lengths, idx_sort = torch.sort(conversation_lengths, dim=0, descending=True)
        # _, idx_unsort = torch.sort(idx_sort, dim=0)
        # packed_conversation = torch.index_select(conversation, dim=1, index=idx_sort)
        # # sort_enc_final_conv size: [layer_num, batch_size, dim]
        # sort_enc_final_conv, memory_bank_conv = self.encoder_conv(packed_conversation, sorted_conv_lengths)
        #
        # # rnn_type == 'GRU':
        # enc_final_conv = torch.index_select(sort_enc_final_conv, dim=1, index=idx_unsort)

        # pack conversation and forward into rnn
        sorted_conv_lengths, idx_sort = torch.sort(conversation_lengths, dim=0, descending=True)
        _, idx_unsort = torch.sort(idx_sort, dim=0)
        sorted_conv_emb = torch.index_select(conv_emb, dim=1, index=idx_sort)

        packed_conv_emb = sorted_conv_emb
        if conversation_lengths is not None and not self.no_pack_padded_seq:
            # Lengths data is wrapped inside a Tensor.
            sorted_conv_lengths = sorted_conv_lengths.view(-1).tolist()
            packed_conv_emb = pack(sorted_conv_emb, sorted_conv_lengths)

        conv_memory_bank, conv_encoder_final = self.rnn(packed_conv_emb, encoder_state)

        if conversation_lengths is not None and not self.no_pack_padded_seq:
            conv_memory_bank = unpack(conv_memory_bank)[0]

        conv_memory_bank = torch.index_select(conv_memory_bank, dim=1, index=idx_unsort)  # assume rnn_type == 'GRU'
        conv_encoder_final = torch.index_select(conv_encoder_final, dim=1, index=idx_unsort)  # assume rnn_type == 'GRU'

        # if self.use_bridge:
        #     src_encoder_final = self._bridge(src_encoder_final)
        #     conv_encoder_final = self._bridge(conv_encoder_final)

        assert src_encoder_final.size() == conv_encoder_final.size()
        pre_encoder_final = torch.cat([src_encoder_final, conv_encoder_final], dim=2)  # [4, 16, 300]
        batch_size = pre_encoder_final.size()[1]
        reshape_encoder_final = pre_encoder_final.transpose(0, 1).contiguous().view(batch_size, -1)  # [16, 1200]
        encoder_final = self.combine_two_enc_state(reshape_encoder_final)

        return encoder_final.unsqueeze(0), src_memory_bank  # the second return is unused later
...
class TwoRNNEncoderConcat(EncoderBase):
    """
    code from wchen
    ...
    def __init__(self, rnn_type, bidirectional, num_layers, hidden_size,
                 dropout, embeddings):
        super(RNNTwoEncoderConcat, self).__init__()
        ...
        # self.ans_rnn_list = nn.ModuleList([getattr(nn, rnn_type)(  # getattr(nn, rnn_type) = torch.nn.modules.rnn.LSTM
        #     input_size=embeddings.embedding_size,
        #     hidden_size=hidden_size,
        #     num_layers=1,
        #     bidirectional=self.bidirectional) for _ in input_size_list])
        # self.match_fc = nn.Linear(2 * hidden_size, 2 * hidden_size, bias=False)
        self.combine_output = nn.Linear(2 * hidden_size, 2 * hidden_size, bias=True)
        self.combine_hidden = nn.Linear(2 * hidden_size, hidden_size, bias=True)
        # self.gate_fcs_list = nn.ModuleList([nn.Linear(4 * hidden_size, 2 * hidden_size) for _ in range(1)])
        # self.merge_fc = nn.Linear(6 * hidden_size, 2 * hidden_size)
        # self.tanh = nn.Tanh()
    ...
    def forward(self, src_input, ans_input, lengths=None, hidden=None):
        ...
        s_len, batch, emb_dim = src_input.size()
        ...
        for layer_idx in range(len(self.src_rnn_list)):
            ...
            outputs = self.combine_output(torch.cat([src_outputs, ans_outputs], dim=0))  # [src_seq_len+ans_seq_len, batch_size, 2 * hidden]
...
class MatchRNNEncoder(EncoderBase):
    ...
    def forward(self, src_input, ans_input, lengths=None, hidden=None):
        ...
        def ans_match(src_seq, ans_seq):
            import torch.nn.functional as F
            # BF_src_mask = sequence_mask(src_lengths)  # [batch, src_len]
            ...
            # BF_ans_outputs = self.match_fc(BF_ans_outputs)

            # compute scores
            scores = BF_src_outputs.bmm(BF_ans_outputs.transpose(2, 1))  # [batch, src_seq_len, ans_seq_len]

            # mask padding
            Expand_BF_ans_mask = BF_ans_mask.unsqueeze(1).expand(scores.size())  # [batch, src_seq_len, ans_seq_len]
            scores.data.masked_fill_((1 - Expand_BF_ans_mask).byte(), -float('inf'))
            ...
            # # get the max attentive vector
            # _, max_index = torch.max(scores, dim=-1)    # [batch, src_seq_len]
            # max_att_vec = torch.cat([BF_ans_outputs[i, :, :].index_select(0, max_index[i, :]).unsqueeze(0)
            #                          for i in range(batch)], dim=0)
            # max_att_vec = max_att_vec.transpose(0, 1)   # [src_seq_len, batch, 2*hidden_size]
            return matched_seq
        ...
        for layer_idx in range(len(self.src_rnn_list)):
            ...
            # ans_outputs, _ = ans_rnn(ans_emb (no sorting), hidden)
            ...
            # forward
            # separate BiRNN
            # ans_outputs, ans_hidden = ans_rnn(packed_ans_emb, hidden)
            # shared BiRNN
            ans_outputs, ans_hidden = src_rnn(packed_ans_input, hidden)
            if sorted_ans_lengths is not None and not self.no_pack_padded_seq:
                ans_outputs = unpack(ans_outputs)[0]
            ...
            matched_seq = ans_match(src_outputs, ans_outputs)

            src_outputs = torch.cat([src_outputs, matched_seq], dim=-1)

            # later dropout
            # src_outputs = self.dropout(src_outputs)
            # only change the src input
            src_input = src_outputs
            ans_input = ans_outputs
        ...
        if src_lengths is not None and not self.no_pack_padded_seq:
            ...
            packed_input = pack(src_input, src_lengths.view(-1).tolist())
        # forward
        src_outputs, src_hidden = self.merge_rnn(packed_input, hidden)
        if src_lengths is not None and not self.no_pack_padded_seq:
            # output
            src_outputs = unpack(src_outputs)[0]

        return src_hidden, src_outputs


class MatchRNNEncoder1(EncoderBase):
    """
    code from wchen
    Match Bi-directional RNN, Reading Comprehension like RNN encoder
    (src -> BiRNN1, ans -> BiRNN3) -> match layer -> BiRNN2->Decoder
    ...
    def __init__(self, rnn_type, bidirectional, num_layers, hidden_size,
                 dropout, embeddings):
        super(MatchRNNEncoder1, self).__init__()
    ...
    def forward(self, src_input, ans_input, lengths=None, hidden=None):
        ...
        for layer_idx in range(len(self.src_rnn_list)):
            ...
            src_outputs = torch.cat([src_outputs, matched_seq], dim=-1)
            src_hidden = torch.cat([src_hidden, ans_hidden], dim=-1)

            src_outputs = self.combine_output(src_outputs)
            src_hidden = self.combine_hidden(src_hidden)

        return src_hidden, src_outputs


class MatchRNNEncoder2(EncoderBase):
    """
    code from wchen
    Match Bi-directional RNN, Reading Comprehension like RNN encoder
    (src -> BiRNN1, ans -> BiRNN3) -> match layer -> BiRNN2->Decoder
    ...
    def __init__(self, rnn_type, bidirectional, num_layers, hidden_size,
                 dropout, embeddings):
        super(MatchRNNEncoder2, self).__init__()
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None, max_src_len=None):
        """
...
class MemoryModel(nn.Module):
    ...
    def forward(self, src, conversation, tgt, lengths, max_src_len, max_conv_len, dec_state=None):
        """Forward propagate a `src` and `tgt` pair for training.
        Possible initialized with a beginning decoder state.
        ...
        enc_final, memory_bank = self.encoder(src, conversation, lengths, max_src_len, max_conv_len)
        enc_state = \
            self.decoder.init_decoder_state(src, memory_bank, enc_final)
        src_lengths = lengths[0]
        decoder_outputs, dec_state, attns = \
            self.decoder(tgt, memory_bank,
                         enc_state if dec_state is None
                         else dec_state,
    ...
    def forward(self, src, conversation, tgt, lengths, max_src_len=None, max_conv_len=None, dec_state=None):
        ...
        decoder_outputs, dec_state, attns = \
            self.decoder(tgt, memory_bank,
                         ...
                         memory_lengths=lengths)  # only for match_encoder2
        if self.multigpu:
            # Not yet supported on multi-gpu
            dec_state = None
            attns = None
